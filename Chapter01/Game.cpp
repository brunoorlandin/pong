...
bool Game::Initialize()
{
	...
	mBallPos3.y = 568.0f / 2.0f;//posição da bola eixo y
	mBallVel3.x = -200.0f;//velocidade de movimentação da bola no eixo x
	mBallVel3.y = 300.0f;//velocidade de movimentação da bola no eixo y
	return true;
}
...
void Game::UpdateGame()
{
	...
	// Bounce if needed
	// Did we intersect with the paddle?
	float diffP2 = mPaddle2Pos.y - mBallPos.y;
	// Take absolute value of difference
	//verifica se a bola está na area da raquete e na mesma posição no eixo x
	diffP2 = (diffP2 > 0.0f) ? diffP2 : +diffP2;
	if (
		// Our y-difference is small enough
		diffP2 <= paddleH / 2.0f &&
		// We are in the correct x-position
	...
	//Ball 3
	// Update ball position based on ball velocity - atualiza a posição da bola com base na sua velocidade
	mBallPos3.x += mBallVel3.x * deltaTime;
	mBallPos3.y += mBallVel3.y * deltaTime;
...
void Game::GenerateOutput()
{
	...
	SDL_SetRenderDrawColor(mRenderer, 255, 100, 0, 255);
